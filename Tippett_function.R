Tippett_plot = function(LR.H1.exp, LR.H2.exp, title="")
{
  ...
  plot(main=title, x.range, Tippett.2, type="s", xlab=expression(paste(log[10],"LR greater than")), ylab="Proportion of cases [%]", xlim=c(-3,3), ylim=c(0,100), lty=3)
  par(new=TRUE)
  plot(x.range, Tippett.1, type="s", xlab=expression(paste(log[10],"LR greater than")), ylab="Proportion of cases [%]", xlim=c(-3,3), ylim=c(0,100))
  legend("bottomleft", c(expression(paste("true-",H[2]," LR values")), expression(paste("true-",H[1]," LR values"))), lty=c(3,1), bty="n")
